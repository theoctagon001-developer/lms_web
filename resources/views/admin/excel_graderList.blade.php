...
<body class="bg-gradient-to-r from-blue-300 via-blue-200 to-blue-100 min-h-screen p-0 m-0">
    ...
    <div class="max-w-4xl mx-auto bg-white shadow-md rounded-lg p-6 mt-6 text-center">
        ...
        <h2 class="text-2xl font-bold text-gray-700 mb-4">Upload Grader List with Teacher Allocations</h2>
...
<script>
    ...
    document.getElementById("submitButton").addEventListener("click", async function() {
        ...
        try {
            initializeApiBaseUrl();
            let response = await fetch(`${API_BASE_URL}api/Uploading/excel-uploading/graders-assign`, {
                method: "POST"
                , body: formData
            , });
                        ...
                        <tbody>
                            ${success.map(record => `
                                <tr class="bg-green-100">
                                    ...
                                    <td class="border p-2 text-center">${record.message}</td>
                                <td class="border p-2 text-center">${record.teacher}</td>
                                <td class="border p-2 text-center">${record.session}</td>
                                </tr>
                            `).join('')}
                        </tbody>
                    </table>
                        ...
                        <tbody>
                            ${error.map(record => `
                                <tr class="bg-red-100">
                                    <td class="border p-2 text-center">${record.status}</td>
                                    <td class="border p-2 text-center">${record.message}</td>
                                     <td class="border p-2 text-center">${record?.teacher??'N/A'}</td>
                                      <td class="border p-2 text-center">${record?.session??'N/A'}</td>
                                </tr>
                            `).join('')}
                        </tbody>
                    </table>
