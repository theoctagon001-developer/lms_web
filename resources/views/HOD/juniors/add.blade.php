...
<head>
    ...
    <title>Add New Junior Lecturer</title>
    ...
    <div class="flex flex-1 overflow-hidden">
        <div class="flex-1 overflow-auto">
            <div class="container mx-auto px-4 py-6">
                <!-- Screen Title -->
                <h1 class="screen-title">Add New Junior Lecturer</h1>
                ...
                <div id="loading-overlay" class="fixed inset-0 bg-gray-500 bg-opacity-50 flex items-center justify-center z-50 hidden">
                    <div class="bg-white rounded-lg p-6 shadow-lg flex items-center space-x-4">
                        ...
                        <span class="text-gray-700 font-medium">Saving junior lecturer...</span>
                    </div>
                </div>

                <!-- Main Form Container -->
                <div class="bg-white rounded-lg shadow-md p-6 animate-fade-in">
                    <form id="add-junior-form" class="space-y-6">
                        <!-- Grid layout for form fields -->
                        <div class="form-grid grid grid-cols-1 md:grid-cols-2 gap-6">
                            <!-- Junior Lecturer Name -->
                            <div>
...
const ADD_JUNIOR_API_URL = `${API_BASE_URL}api/Hod/add-junior`;

// DOM Elements
const loadingOverlay = document.getElementById('loading-overlay');
const addJuniorForm = document.getElementById('add-junior-form');
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    addJuniorForm.addEventListener('submit', handleFormSubmit);
...
async function handleFormSubmit(e) {
    ...
    // Get form data
    const formData = new FormData(addJuniorForm);
    ...
    try {
        const response = await fetch(ADD_JUNIOR_API_URL, {
            method: 'POST',
            headers: {
        ...
        if (!response.ok) {
            if (response.status === 422) {
                ...
                // Duplicate junior lecturer
                showToast(data.message, 'error');
                if (data.message.includes('cnic')) {
                    markFieldAsError('cnic', 'Employee ID already exists');
                } else {
                    markFieldAsError('name', 'Junior lecturer with this name already exists');
                }
            } else {
                // Other errors
                showToast(data.message || 'Failed to add junior lecturer', 'error');
            }
            return;
        }
        
        // Success
        showToast('Junior lecturer added successfully!', 'success');
        
        // Show success message with credentials
        const successMessage = `Junior lecturer added successfully!<br>
