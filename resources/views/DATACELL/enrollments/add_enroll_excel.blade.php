...
<head>
    ...
    <title>Upload Student Enrollments</title>
...
<body class="bg-gray-50">
    ...
    <div class="container mx-auto px-4 py-8">
        <!-- Screen Title -->
        <h1 class="text-2xl sm:text-3xl font-bold text-blue-800 text-center mb-6">Upload Student Enrollments</h1>

        <!-- Current Session Display -->
        <div class="max-w-4xl mx-auto bg-blue-600 text-white text-center py-2 text-lg font-semibold shadow-md rounded-t-lg">
        ...
        <div class="upload-container animate-fade-in">
            ...
            <div class="session-dropdown-container">
                ...
                <select id="sessionDropdown" class="session-dropdown focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <div class="mt-4">
                ...
                <div class="file-upload" id="file-upload-area">
                    ...
                    <a href="/EnrollmentsExcel.xlsx" class="sample-file-link" download>
    ...
    <script>
            ...
            try {
                ...
                if (Array.isArray(data)) {
                    ...
                    if (sessionDropdown.options.length > 0) {
                        sessionDropdown.options[0].selected = true; // Select the first session by default
                    }
                } else {
                    console.error("Failed to fetch sessions or data format is incorrect");
                    showToast('Failed to load sessions data', 'error');
                }
            } catch (error) {
                console.error("Error fetching session data:", error);
                showToast('Error loading sessions data', 'error');
            }
        }
        ...
        async function handleFileUpload() {
            ...
            try {
                 API_BASE_URL = await getApiBaseUrl();
                const response = await fetch(`${API_BASE_URL}api/Uploading/excel-upload/student-enrollments`, {
                    method: 'POST',
                    body: formData
                });
                ...
                if (response.ok) {
                    ...
                    showToast(`Upload Successful! Total Records: ${result.data["Total Student Rows"]}, Added: ${result.data["Added Enrollments"]}, Failed: ${result.data["Failed Enrollments"]}`, 'success');
                } else {
        ...
        function displayUploadResults(data) {
            ...
            if (data.data.Sucess && data.data.Sucess.length > 0) {
                data.data.Sucess.forEach(item => {
                    const row = document.createElement('tr');
                    row.innerHTML = `
                        <td><span class="status-badge status-success">✅ ${item.status || 'success'}</span></td>
                        <td>${item.message || 'No message available'}</td>
                    `;
                    successTableBody.appendChild(row);
                });
                successCount.textContent = data.data["Added Enrollments"] || data.data.Sucess.length;
            ...
            if (data.data.Error && data.data.Error.length > 0) {
                data.data.Error.forEach(item => {
                    const row = document.createElement('tr');
                    row.innerHTML = `
                        <td><span class="status-badge status-error">❌ ${item.status || 'error'}</span></td>
                        <td>${item.message || 'Unknown error'}</td>
                    `;
                    errorTableBody.appendChild(row);
                });
                errorCount.textContent = data.data["Failed Enrollments"] || data.data.Error.length;
